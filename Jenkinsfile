...
// Generated by: aisland jenkins scaffold --project loggair
// Source template: aisland/services/jenkins.py (JenkinsService.scaffold_pipelines)
// To regenerate:  aisland jenkins scaffold --project loggair --force
// =========================================================================
pipeline {
    agent any
                ...
                stage('Black') {
                    steps {
                        script {
                            ...
                            def targets = sh(script: "for d in loggair tests examples; do if [ -d \"\$d\" ] && find \"\$d\" -name '*.py' | grep -q .; then printf \"%s \" \"\$d\"; fi; done || true", returnStdout: true).trim()
                            if (targets) {
...
" """
                        ...
                        always {
                            script {
                                if (fileExists('isort-checkstyle.xml')) {
                                    recordIssues(
                                        id: 'isort-loggair',
                                        name: 'Isort Import Order (Loggair)',
            ...
            steps {
                script {
                    if (fileExists('tests') && sh(script: "find tests -name '*.py' | grep -q .", returnStatus: true) == 0) {
                        sh "${VENV_BIN}/pytest tests --junitxml=test-report.xml --cov=loggair --cov-report=xml:coverage.xml --cov-report=term"
                    } else {
    ...
    post {
        always {
            echo 'Loggair Pipeline Complete.'
        }
        success {
            echo 'Loggair is healthy.'
        }
        failure {
            echo 'Loggair build failed. Please check linting or test failures.'
        }
    }
}
